...
public class ImpulseEngineClassE : IEngine
{
    ...
    public double? TryCountFuelConsuming(IRouteSegment routeSegment, double mass)
    {
        ...
        {
            totalConsumption += Math.Log(routeSegment.Length) * mass / _commonSpaceEffectiveness;
        }
        ...
        {
            totalConsumption += Math.Log(routeSegment.Length) * mass / _nitrinoParticleNebulaeEffectiveness;
        }
        else
        {
